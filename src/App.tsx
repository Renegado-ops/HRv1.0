...
const ITRHRvBrand: React.FC<{ variant?: 'header' | 'login' }> = ({
  variant = 'header',
}) => (
...
const initialEmployees: Employee[] = [
  {
    ...
    governmentId: '001-1829304-5',
    role: 'User',
    loginMethod: 'Google',
    lastLogin: '24 days ago',
    status: 'Active',
    hireDate: '2024-01-15',
    office: 'Santiago Operations',
  ...
  },
  {
    ...
    governmentId: 'US-PASSPORT-882192',
    role: 'User',
    loginMethod: 'Google',
  ...
  },
  {
    ...
    hireDate: '2023-11-01',
    office: 'Santiago Operations',
    team: 'Accounting & Tax Relief',
  },
  {
    id: '4',
    name: 'Alex Morgan',
    email: 'alex.morgan@company.com',
    governmentId: 'US-DL-9921023',
    role: 'Owner',
    loginMethod: 'Google',
    lastLogin: 'Today',
    status: 'Active',
...
const initialRequests: LeaveRequest[] = [
  ...
  {
    id: '103',
    employeeName: 'Aaron Garcia',
    type: 'Tardanza (Late Arrival)',
    startDate: '2026-09-23',
    endDate: '2026-09-23',
    duration: 'Hourly (Late Arrival)',
    days: 0.25,
    status: 'Approved',
    approver: 'Alex Morgan',
...
const initialOffices: Office[] = [
  {
    ...
    employeesCount: 6,
    type: 'Main Office',
    holidayCalendar: 'US Federal Holidays',
  },
  {
    id: '2',
    ...
    employeesCount: 4,
    type: 'Regional',
    holidayCalendar: 'Dominican Republic National Holidays',
  },
  {
    id: '3',
    name: 'Remote Hub',
    ...
    employeesCount: 2,
    type: 'Remote',
    holidayCalendar: 'International Standard',
  },
];
...
const initialAuditLogs: AuditLog[] = [
  {
    ...
    details: 'Submitted Vacation request for 4 days.',
  },
  {
    id: '2',
    ...
    user: 'Aaron Garcia',
    action: 'Log Tardanza',
    details: 'Logged late arrival (Tardanza - 1.5 hrs late).',
  },
  {
    id: '3',
    timestamp: '2026-09-20 14:02:10',
    user: 'Alex Morgan',
    action: 'Approve Request',
    details: 'Approved Sick leave request for Abby Jhonson.',
  },
];
...
export default function App() {
  ...
  const [loginEmail, setLoginEmail] = useState('alex.morgan@company.com');
  ...
  const [notifications, setNotifications] = useState([
    ...
    },
    {
      ...
      desc: 'Aaron Garcia reported Tardanza for today',
      time: '1h ago',
    },
  ]);
  ...
  const [employees, setEmployees] = useState<Employee[]>(() => {
    const saved = localStorage.getItem('itr_hrv_employees_final_v5');
    return saved ? JSON.parse(saved) : initialEmployees;
  });

  const [requests, setRequests] = useState<LeaveRequest[]>(() => {
    const saved = localStorage.getItem('itr_hrv_requests_final_v5');
  ...
  const [dashboardCalendarFilter, setDashboardCalendarFilter] =
    useState('All Events');
  ...
  const [newUserRole, setNewUserRole] = useState<
    'User' | 'Admin' | 'Accounting' | 'Owner'
  >('User');
  const [newUserOffice, setNewUserOffice] = useState('Headquarters (HQ)');
  ...
  useEffect(() => {
    localStorage.setItem(
      'itr_hrv_employees_final_v5',
      JSON.stringify(employees)
    );
  }, [employees]);
  useEffect(() => {
    localStorage.setItem('itr_hrv_requests_final_v5', JSON.stringify(requests));
  }, [requests]);
  ...
  const handleToggleStatus = (id: string) => {
    ...
    addAuditLog(
      currentUser.name,
      'Toggle Status',
      `Changed ${emp.name}'s status to ${nextStatus}`
    );
  ...
  const handleChangeRole = (
    id: string,
    newRole: 'User' | 'Admin' | 'Accounting' | 'Owner'
  ) => {
    if (!isAdminOrOwner) return;
    const emp = employees.find((e) => e.id === id);
    if (!emp) return;
    ...
    addAuditLog(
      currentUser.name,
      'Change Role',
      `Changed ${emp.name}'s role to ${newRole}`
    );
    setOpenUserMenuId(null);
  };
  ...
  const handleCreateUser = (e: React.FormEvent) => {
    ...
    addAuditLog(
      currentUser.name,
      'Create User',
      `Created user account for ${newUserName} (${newUserGovId}).`
    );
  ...
  const handleCreateRequest = (e: React.FormEvent) => {
    ...
    addAuditLog(
      reqEmployee,
      'Create Request',
      `Submitted ${reqType} (${reqDuration}) for ${calculatedDays} days.`
    );
  ...
  const updateRequestStatus = (
    ...
    if (req) {
      addAuditLog(
        currentUser.name,
        `${newStatus} Request`,
        `${newStatus} ${req.type} request for ${req.employeeName}`
      );
    }
  };

  // --- GUARDAR PROFILE SETTINGS (CON CARGA DE IMAGEN LOCAL) ---
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => {
        setProfileAvatar(reader.result as string); // Convierte la imagen a Base64 y actualiza la vista previa
      };
  ...
  const handleSaveProfile = (e: React.FormEvent) => {
    ...
    addAuditLog(
      currentUser.name,
      'Update Profile',
      'Updated profile settings & avatar.'
    );
  ...
  const downloadReportCSV = (reportTitle: string) => {
    ...
    if (reportTitle === 'Leave & Tardanzas Report') {
      csvData +=
        'Employee,Type,Start Date,End Date,Duration,Days,Status,Reason\n';
      requests.forEach((r) => {
        csvData += `"${r.employeeName}","${r.type}","${r.startDate}","${
          r.endDate
        }","${r.duration}",${r.days},"${r.status}","${r.reason || ''}"\n`;
      });
    } else {
    ...
    link.setAttribute(
      'download',
      `${reportTitle.toLowerCase().replace(/ /g, '_')}_2026.csv`
    );
  ...
  const sortedEmployees = [...employees].filter(
    (emp) =>
  ...
  const renderCalendarCells = () => {
    ...
    for (let i = 0; i < firstDayOfMonth; i++) {
      cells.push(
        <div
          key={`empty-${i}`}
          className="bg-slate-50/50 p-2 min-h-[100px] border-b border-r border-slate-100 rounded-sm"
        ></div>
      );
    }
        ...
        >
          <div className="flex justify-between items-center mb-1">
            <span
              className={`text-xs font-semibold ${
                isToday
                  ? 'bg-sky-500 text-white px-2 py-0.5 rounded-full'
                  : 'text-slate-500'
              }`}
            >
              {day}
          ...
          <div className="space-y-1">
            {dayEvents.map((event) => (
              <div
                key={`${event.id}-${day}`}
                className={`text-[10px] px-1.5 py-1 rounded shadow-sm border font-medium truncate ${
                  event.type === 'Tardanza (Late Arrival)'
              ...
              >
                ...
                {event.type === 'Tardanza (Late Arrival)' ? 'Late' : event.type}
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div
      className="min-h-screen bg-[#f4f7f6] font-sans antialiased text-slate-800"
      onClick={() => {
    ...
    >
            ...
            {isNotifOpen && (
              <div className="absolute right-0 mt-3 w-80 bg-white rounded-md shadow-xl border border-slate-200 z-50 text-slate-800 p-2">
                ...
                {notifications.map((n) => (
                  <div
                    key={n.id}
                    className="p-2 border-b text-xs hover:bg-slate-50"
                  >
      ...
      <main className="p-8 max-w-6xl mx-auto space-y-6">
              ...
              <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-rose-600 mt-1">
                    {
                      requests.filter(
                        (r) => r.type === 'Tardanza (Late Arrival)'
                      ).length
                    }
                  </p>
                </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm space-y-4">
              <div className="flex justify-between items-center border-b pb-4">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">
                    Real-time track for Vacations, PTOs, Sick Leaves, and Late
                    Arrivals (Tardanzas).
                  </p>
                </div>
                <div className="flex items-center space-x-2 text-xs">
                  ...
                  >
                    ...
                    <option value="Tardanza (Late Arrival)">
                      Tardanzas Only
                    </option>
                  </select>
                </div>
              ...
              <div className="divide-y divide-slate-100">
                {requests
                  .filter(
                    (r) =>
                      dashboardCalendarFilter === 'All Events' ||
                      r.type === dashboardCalendarFilter
                  )
                  .map((r) => (
                    <div
                      key={r.id}
                      className="py-3 flex items-center justify-between hover:bg-slate-50 transition px-2 rounded-lg"
                    >
                      <div className="flex items-center space-x-3">
                        <div
                          className={`w-3 h-3 rounded-full ${
                            r.type === 'Tardanza (Late Arrival)'
                              ? 'bg-rose-500'
                              : r.type === 'Vacation'
                              ? 'bg-amber-500'
                              : r.type === 'Sick leave'
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm space-y-4">
              <div className="flex justify-between items-center border-b pb-4">
                <div className="flex items-center space-x-3">
                  ...
                  <span className="font-bold text-slate-800 text-lg w-40 text-center">
                    {monthName}
                  </span>
                  <button
                    onClick={nextMonth}
              ...
              <div className="grid grid-cols-7 border-t border-l border-slate-100 rounded overflow-hidden">
                {[
                  ...
                  <div
                    key={day}
                    className="p-2 text-center text-[10px] font-bold uppercase tracking-wider text-slate-400 border-r border-b border-slate-100 bg-slate-50"
                  >
                    {day}
                  </div>
                ))}
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <select
                ...
                className="border border-gray-300 rounded-lg px-3 py-1.5 text-xs bg-white text-slate-700 font-medium"
              >
              ...
              <button
                ...
                className="bg-[#4a90e2] hover:bg-[#3b7dc9] text-white px-4 py-2 rounded-lg text-xs font-semibold shadow-sm transition flex items-center space-x-2"
              >
                  ...
                  {filteredRequests.map((req) => (
                    <tr key={req.id} className="hover:bg-slate-50">
                      ...
                      <td className="p-3.5 text-xs">
                        {req.startDate} to {req.endDate} ({req.days} d)
                      </td>
                      <td className="p-3.5">
                        <span
                ...
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 space-y-4">
                  <div className="flex justify-between items-center pb-2 border-b border-slate-100">
                    <div className="relative w-80">
                      <input
                        type="text"
                        placeholder="Search by name, email or Cedula/ID..."
                    ...
                    <span className="text-xs text-gray-500 font-medium">
                      Showing {sortedEmployees.length} users
                    </span>
                  </div>
                      ...
                      {sortedEmployees.map((emp) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="p-3 text-xs font-mono font-medium text-slate-700">
                              {emp.governmentId}
                            </td>
                            ...
                            <td className="p-3 text-center relative">
                              ...
                              {openUserMenuId === emp.id && isAdminOrOwner && (
                                <div
                                  onClick={(e) => e.stopPropagation()}
                                  className="absolute right-4 top-10 w-60 bg-white rounded-xl shadow-2xl border border-slate-200 z-50 text-left py-1 text-xs divide-y divide-slate-100"
                                >
                                  <div className="py-1">
                                    <button
                                  ...
                                  <div className="py-1">
                                    ...
                                    {(
                                      ...
                                      <button
                                        key={roleChoice}
                                        onClick={() =>
                                          handleChangeRole(emp.id, roleChoice)
                                        }
                                        className={`w-full text-left px-3 py-1.5 text-xs hover:bg-slate-50 transition flex items-center justify-between ${
                                          emp.role === roleChoice
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                    <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-100 space-y-1">
                      ...
                      <p className="text-gray-500 leading-relaxed">
                        Acceso a autoservicio básico. Puede solicitar días
                        libres, ver sus propios balances y consultar el
                        directorio general de empleados.
                      </p>
                    </div>
                    ...
                    <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-100 space-y-1">
                      ...
                      <p className="text-gray-500 leading-relaxed">
                        Acceso a reportes de horas de trabajo, exportación de
                        listas de ausencias a CSV y revisión de acumulación de
                        vacaciones para nómina.
                      </p>
                    </div>
                    ...
                    <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-100 space-y-1">
                      ...
                      <p className="text-gray-500 leading-relaxed">
                        Control operativo total. Puede crear/editar usuarios,
                        aprobar/rechazar solicitudes de cualquier equipo,
                        administrar oficinas y asignar roles.
                      </p>
                    </div>
                    ...
                    <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-100 space-y-1">
                      ...
                      <p className="text-gray-500 leading-relaxed">
                        Privilegios globales máximos de la organización. Incluye
                        auditoría del sistema (Audit Trail), gestión
                        administrativa y control de la cuenta.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  <div className="space-y-6">
                    <div className="bg-white rounded-xl border p-6 shadow-sm flex flex-col items-center text-center">
                      ...
                      <p className="text-xs text-gray-400">
                        Hire date: {activeEmployee?.hireDate}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="lg:col-span-2 space-y-6">
                    <div className="bg-white rounded-xl border p-6 shadow-sm space-y-4">
                      <h3 className="text-base font-semibold text-slate-700 border-b pb-3">
                        Personal & Legal Information
                      </h3>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-y-3 text-xs items-center">
                        <span className="font-medium text-slate-600">
                          Cedula / Government ID:
                        </span>
                        <div className="md:col-span-2 bg-slate-50 border rounded px-3 py-1.5 font-mono font-bold text-slate-800">
                          {activeEmployee?.governmentId}
                        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {teams.map((t) => (
                ...
                >
                  ...
                  <p className="text-xs font-semibold text-sky-600 pt-2 border-t">
                    Team Lead: {t.lead}
                  </p>
                </div>
              ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {offices.map((o) => (
                ...
                >
                  ...
                  <span className="inline-block text-[10px] font-bold px-2 py-0.5 rounded bg-sky-100 text-sky-800">
                    {o.holidayCalendar}
                  </span>
                </div>
              ))}
        ...
        {activeTab === 'Reports' && (
          <div className="space-y-6">
            <h2 className="text-2xl font-normal text-slate-700">
              System Reports & Payroll Export
            </h2>
            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
              <p className="text-xs text-gray-500">
                Export structured data for payroll, compliance, or HR audit
                purposes.
              </p>
              <div className="flex space-x-3">
                <button
                  onClick={() => downloadReportCSV('Leave & Tardanzas Report')}
                  className="bg-[#1ba0d7] hover:bg-[#188db8] text-white px-4 py-2 rounded-lg text-xs font-semibold flex items-center space-x-2 transition shadow-sm"
                >
                  <Download className="w-4 h-4" />
                  <span>Export Leave & Tardanzas CSV</span>
                </button>
                <button
                  onClick={() => downloadReportCSV('Employee Master Report')}
                  className="bg-slate-800 hover:bg-slate-900 text-white px-4 py-2 rounded-lg text-xs font-semibold flex items-center space-x-2 transition shadow-sm"
                >
                  <Download className="w-4 h-4" />
                  <span>Export Employee Master CSV</span>
                </button>
              </div>
            </div>
          </div>
        ...
        {activeTab === 'Settings' && (
          <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm text-slate-600 text-xs leading-relaxed">
              Configuration Panel for ITR HRv System Settings, Google Workspace
              integration, and leave policies.
            </div>
          </div>
        )}
      </main>
            ...
            <form onSubmit={handleSaveProfile} className="space-y-4 text-xs">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center space-x-4">
                  <img
                    src={profileAvatar}
                    alt="Preview"
                    className="w-14 h-14 rounded-full border-2 border-sky-500 object-cover shadow-sm"
                  />

                  <div className="flex-1 flex flex-col justify-center">
                    <label className="cursor-pointer bg-slate-100 hover:bg-slate-200 border border-slate-300 text-slate-700 px-3 py-1.5 rounded-lg text-xs font-medium transition inline-flex items-center space-x-2 w-fit">
              ...
              <div className="space-y-2 pt-2 border-t">
                ...
                <input
                  ...
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-xs focus:outline-sky-500"
                />
              </div>
              ...
              <div className="space-y-2 pt-2 border-t">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full border border-gray-300 rounded-lg px-3 py-1.5 text-xs focus:outline-sky-500"
                  />
                </div>
              ...
              <div className="flex justify-end space-x-2 pt-3 border-t">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 text-xs bg-[#0066ff] hover:bg-[#0052cc] text-white rounded-lg font-medium shadow-sm transition"
                >
                  Save Changes
                </button>
              </div>
            ...
            <form onSubmit={handleCreateRequest} className="space-y-4 text-xs">
              ...
              <div>
                ...
                >
                  ...
                  <option value="Hourly (Late Arrival)">
                    Hourly (Late Arrival)
                  </option>
                </select>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Reason for request or tardanza..."
                  className="w-full border rounded-lg px-3 py-1.5 text-xs"
                />
              </div>
              ...
              <div className="flex justify-end space-x-2 pt-3 border-t">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 text-xs bg-[#4a90e2] text-white rounded-lg font-medium shadow-sm"
                >
                  Submit Request
                </button>
              </div>
      ...
      {/* MODAL AGREGAR USUARIO CON CÉDULA */}
      {isUserModalOpen && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6 space-y-4">
            <div className="flex justify-between items-center border-b pb-3">
            ...
            <form onSubmit={handleCreateUser} className="space-y-4 text-xs">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="402-XXXXXXX-X or US-Passport"
                />
              </div>
